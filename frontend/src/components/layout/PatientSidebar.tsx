...
const primaryNav = [
  ...
  { label: 'Start Intake', icon: Stethoscope },
    ...
    <aside className="hidden min-h-screen w-[272px] shrink-0 flex-col bg-brand-700 px-2 py-8 text-white lg:flex">
      ...
      <nav className="space-y-1" aria-label="Patient portal">
        {primaryNav.map((item) => {
          ...
          const isStart = item.label === 'Start Intake';
          return (
            <button
              className={`flex min-h-11 w-full items-center gap-3 rounded-full px-4 text-left text-sm font-medium transition ${
      ...
      <div className="mt-6 border-t border-white/35 pt-5">
        ...
        <div className="space-y-1">
          <a className="flex min-h-11 items-center gap-3 rounded-full px-4 text-sm font-medium text-white/70 hover:bg-white/10" href="#">
      ...
      <div className="mt-auto space-y-1">
        {secondaryNav.map((item) => {
          const Icon = item.icon;
          return (
            <a className="flex min-h-11 items-center gap-3 rounded-full px-4 text-sm font-medium text-white/70 hover:bg-white/10" href="#" key={item.label}>
              <Icon size={20} strokeWidth={1.9} />
              {item.label}
            </a>
          );
