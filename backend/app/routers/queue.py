...
def _raise_http_error(exc: Exception) -> None:
    ...
    if isinstance(exc, QueueDatabaseError):
        raise HTTPException(
            ...
            detail="Queue database operation failed.",
        ) from exc
    if isinstance(exc, QueueConfigurationError):
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Queue database is not configured.",
        ) from exc
    if isinstance(exc, QueueDataError):
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Queue data could not be processed.",
        ) from exc
    raise exc
...
@router.get("/seen-today", response_model=list[PatientRecord])
...
@router.patch("/{patient_id}/status", response_model=PatientRecord)
...
@router.post("/{patient_id}/override", response_model=PatientRecord)
async def override_urgency(
    ...
    try:
        return await queue_service.override_urgency(
            patient_id,
            request.level,
            current_user.id,
        )
...
@router.post("/{patient_id}/seen", response_model=PatientRecord)
async def mark_seen(
    patient_id: str,
